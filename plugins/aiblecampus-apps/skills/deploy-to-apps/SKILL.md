...
- "산출물 공개해줘": `set_artifact_visibility`로 갤러리에 공개한다. 팀 관리자 권한이 필요하다.
- "발표자료 올려줘": 프로젝트에서 PDF를 찾아 `upload_artifact_slides`로 올린다. 여러 개면 어느 것인지 한 번 묻는다.
...
해커톤 산출물을 다른 참가자와 심사위원이 볼 수 있게 공개한다. 갤러리는 조직(고객사) 단위이고 조직에 붙은 팀의 공개 앱만 실린다.

- 공개 전환은 `set_artifact_visibility`다. 제목과 설명을 함께 정하면 갤러리 카드에 그대로 보인다.
- 발표자료는 `upload_artifact_slides`에 PDF 절대 경로를 준다. 서버가 페이지 이미지로 바꾸고 첫 장이 카드 대표 이미지가 된다.
- PPT나 Keynote는 받지 않는다. 사용자에게 PDF로 내보내 달라고 한 번만 안내한다.
- 둘 다 팀 관리자 이상만 할 수 있다. 권한이 없으면 팀 관리자에게 요청하라고 알린다.
- 공개는 갤러리 노출 여부다. 앱 주소 자체는 원래 접속 가능하므로 "공개로 바꾸면 그때부터 접속이 열린다"고 설명하지 않는다.
- 앱이 개인 작업 공간에 있으면 갤러리에 실리지 않는다. 팀으로 옮겨야 한다고 알린다.
