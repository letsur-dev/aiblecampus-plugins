...
- Docker를 실행할 수 있으면 배포할 Dockerfile로 빌드하고 같은 실행 명령과 비특권 사용자로 주요 페이지, API 및 파일 접근을 검증한다. 사용할 수 없으면 로컬 빌드와 정적 검증을 수행하고 컨테이너 검증을 했다고 보고하지 않는다.
- 최종 응답의 URL에 HTTPS 요청을 보내 확인한다. 인증서 오류 검사를 끄지 않으며 연결, 인증서, HTTP 상태 오류를 구분한다. 앱이 의도한 로그인 리다이렉트는 허용하지만 5xx 또는 다른 앱의 화면을 성공으로 보지 않는다.
- `public-tls-not-ready`는 플랫폼 인증서 준비 문제다. 코드나 메모리를 변경하지 말고 상태와 로그를 조회한다. 발급 제한 또는 DNS 문제는 운영자가 처리해야 하며 동일 요청을 반복 배포하지 않는다.
